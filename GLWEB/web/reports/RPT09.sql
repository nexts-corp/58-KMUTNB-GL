				...
				WHERE
					ACC.ACCOUNTID IN (
						SELECT
							ACCOUNTID
						FROM
							MASTER3D.ACCOUNT START WITH MASTERID = 1120400000 CONNECT BY PRIOR ACCOUNTID = MASTERID
					)
				AND GL.GLHEADSTATUS != 'V'
				AND ACC.ACCOUNTSTATUS = 'Y'
				AND GLHEAD.GLHEADDATE <= TO_DATE (
				...
				WHERE
					ACC.ACCOUNTID IN (
						SELECT
							ACCOUNTID
						FROM
							MASTER3D.ACCOUNT START WITH MASTERID = 1180006000 CONNECT BY PRIOR ACCOUNTID = MASTERID
					)
				AND GLHEAD.GLHEADDATE <= TO_DATE (
					'{{DATE_END}}',
					'DD/MM/YYYY'
				...
				WHERE
					ACC.ACCOUNTID IN (
						SELECT
							ACCOUNTID
						FROM
							MASTER3D.ACCOUNT START WITH MASTERID = 1180008000 CONNECT BY PRIOR ACCOUNTID = MASTERID
					)
				AND GL.GLHEADSTATUS != 'V'
				AND ACC.ACCOUNTSTATUS = 'Y'
				AND GLHEAD.GLHEADDATE <= TO_DATE (
				...
				WHERE
					ACC.ACCOUNTID IN (
						SELECT
							ACCOUNTID
						FROM
							MASTER3D.ACCOUNT START WITH MASTERID = 2200001000 CONNECT BY PRIOR ACCOUNTID = MASTERID
					)
				AND GL.GLHEADSTATUS != 'V'
				AND ACC.ACCOUNTSTATUS = 'Y'
				AND GLHEAD.GLHEADDATE <= TO_DATE (
				...
				WHERE
					ACC.ACCOUNTID IN (
						SELECT
							ACCOUNTID
						FROM
							MASTER3D.ACCOUNT START WITH MASTERID = 2200005000 CONNECT BY PRIOR ACCOUNTID = MASTERID
					)
				AND GL.GLHEADSTATUS != 'V'
				AND ACC.ACCOUNTSTATUS = 'Y'
				AND GLHEAD.GLHEADDATE <= TO_DATE (
				...
				WHERE
					ACC.ACCOUNTID IN (
						SELECT
							ACCOUNTID
						FROM
							MASTER3D.ACCOUNT START WITH MASTERID = 2200006000 CONNECT BY PRIOR ACCOUNTID = MASTERID
					)
				AND GL.GLHEADSTATUS != 'V'
				AND ACC.ACCOUNTSTATUS = 'Y'
				AND GLHEAD.GLHEADDATE <= TO_DATE (
				...
				WHERE
					ACC.ACCOUNTID IN (
						SELECT
							ACCOUNTID
						FROM
							MASTER3D.ACCOUNT START WITH MASTERID = 2200004000 CONNECT BY PRIOR ACCOUNTID = MASTERID
					)
				AND GL.GLHEADSTATUS != 'V'
				AND ACC.ACCOUNTSTATUS = 'Y'
				AND GLHEAD.GLHEADDATE <= TO_DATE (
				...
				WHERE
					ACC.ACCOUNTID IN (
						SELECT
							ACCOUNTID
						FROM
							MASTER3D.ACCOUNT START WITH MASTERID = 2200008000 CONNECT BY PRIOR ACCOUNTID = MASTERID
					)
				AND GL.GLHEADSTATUS != 'V'
				AND ACC.ACCOUNTSTATUS = 'Y'
				AND GLHEAD.GLHEADDATE <= TO_DATE (
				...
				WHERE
					ACC.ACCOUNTID IN (
						SELECT
							ACCOUNTID
						FROM
							MASTER3D.ACCOUNT START WITH MASTERID = 2200009000 CONNECT BY PRIOR ACCOUNTID = MASTERID
					)
				AND GL.GLHEADSTATUS != 'V'
				AND ACC.ACCOUNTSTATUS = 'Y'
				AND GLHEAD.GLHEADDATE <= TO_DATE (
				...
				WHERE
					ACC.ACCOUNTID IN (
						SELECT
							ACCOUNTID
						FROM
							MASTER3D.ACCOUNT START WITH MASTERID = 2200011000 CONNECT BY PRIOR ACCOUNTID = MASTERID
					)
				AND GL.GLHEADSTATUS != 'V'
				AND ACC.ACCOUNTSTATUS = 'Y'
				AND GLHEAD.GLHEADDATE <= TO_DATE (
						...
						WHERE
							GL.ACCOUNTID IN (
								SELECT
									ACCOUNTID
								FROM
									MASTER3D. ACCOUNT START WITH MASTERID >= 1240100000
								AND MASTERID < 1240500000
								AND NATURETYPE = 'D' CONNECT BY PRIOR ACCOUNTID = MASTERID
							)
						AND GL.GLHEADSTATUS != 'V'
						AND ACC.ACCOUNTSTATUS = 'Y'
						AND GLHEAD.GLHEADDATE <= TO_DATE (
					...
					WHERE
						GL.ACCOUNTID IN (
							...
							FROM
								MASTER3D. ACCOUNT START WITH MASTERID >= 1240100000
							AND MASTERID < 1240500000
							AND NATURETYPE = 'C' CONNECT BY PRIOR ACCOUNTID = MASTERID
						)
					AND GL.GLHEADSTATUS != 'V'
					AND ACC.ACCOUNTSTATUS = 'Y'
					AND GLHEAD.GLHEADDATE <= TO_DATE (
										...
										(
											...
											FROM
												(
													SELECT
														'1' AS MASK,
														CASE WHEN SUM (DR - CR) IS NOT NULL THEN SUM (DR - CR) ELSE 0 END AS BALANCE_CURRENT
													FROM
														MASTER3D.GL
													LEFT OUTER JOIN MASTER3D.GLHEAD ON GLHEAD.GLHEADID = GL.GLHEADID
													LEFT OUTER JOIN MASTER3D. ACCOUNT ACC ON ACC.ACCOUNTID = GL.ACCOUNTID
											...
											FULL OUTER JOIN (
												SELECT
													'1' AS MASK,
													CASE WHEN SUM (DR - CR) IS NOT NULL THEN SUM (DR - CR) ELSE 0 END AS BALANCE_PAST
												FROM
													MASTER3D.GL
												LEFT OUTER JOIN MASTER3D.GLHEAD ON GLHEAD.GLHEADID = GL.GLHEADID
												LEFT OUTER JOIN MASTER3D. ACCOUNT ACC ON ACC.ACCOUNTID = GL.ACCOUNTID
