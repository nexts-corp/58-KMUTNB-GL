SELECT ROW_NUMBER () OVER (ORDER BY NULL) AS ID,PLANID,PLANNAME,DEPARTMENTID,DEPARTMENTNAME,ACCOUNTID,ACCOUNTNAME ,SUM(DR) AS DR,SUM(CR) AS CR FROM 
  (
    SELECT
      gl1.PLANID,
      pla.PLANNAME,
  ...
  ) GROUP BY DEPARTMENTNAME,DEPARTMENTID,PLANNAME,PLANID,ACCOUNTNAME,ACCOUNTID ORDER BY ACCOUNTID